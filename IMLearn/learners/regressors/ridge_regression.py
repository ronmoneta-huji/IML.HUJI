...
class RidgeRegression(BaseEstimator):
    ...
    def _fit(self, X: np.ndarray, y: np.ndarray) -> NoReturn:
        ...
        """
        # X_lam = np.concatenate((X, (np.sqrt(self.lam_) * np.identity(X.shape[1]))), axis=0)
        # y_lam = np.concatenate((y, np.zeros(X.shape[1])), axis=0)
        ...
        iden[0, 0] = 0
        self.coefs_ = np.linalg.inv(X.T @ X + (self.lam_ * iden)) @ X.T @ y
